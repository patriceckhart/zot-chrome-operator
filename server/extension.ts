...
function postAction(action: Record<string, unknown>): Promise<any> {
...
function formatPageContext(ctx: any): string {
  let out = ""
...
send({ type: "hello", name: "zot-chrome", version: "0.1.0", capabilities: ["tools"] })
send({
  type: "register_tool",
  name: "browser_action",
...
rl.on("line", async (line) => {
  ...
  try {
    ...
    const payload: Record<string, unknown> = { type: args.action }
